import * as ISP from "./interfaces/ISamehadakuParser.js";
import * as ISPE from "./interfaces/ISamehadakuParserExtra.js";
import type { Server } from "@interfaces/IGlobal.js";
...
export default class SamehadakuParser extends SamehadakuParserExtra {
  ...
  async parseServerUrl(serverId: string, originUrl: string): Promise<ISP.ServerUrl> {
    ...
    // =================== PERUBAHAN DI SINI ===================
    // Panggil belloFetch dan simpan hasilnya
    const resultUrl = await belloFetch(
    ...
    // Cek hasilnya setelah dipanggil
    if (!resultUrl) setResponseError(400);
    // =========================================================

    data.url = this.generateSrcFromIframeTag(resultUrl);
